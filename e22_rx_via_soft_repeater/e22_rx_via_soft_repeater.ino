 ...
 * Пакеты приходят через ПРОГРАММНЫЙ репитер (см. e22_soft_repeater_N16R8.ino):
 *   репитер сам является узлом сети с адресом 00:02 и, приняв фиксированное
 *   сообщение от передатчика, пересылает его дальше на адрес этого приёмника
 *   (00:03) уже отдельной командой sendFixedMessage().
 *
 * Т.к. маршрутизация теперь делается скетчем репитера (а не аппаратным
 * REG0/REG3 модуля), передатчик, репитер и этот приёмник должны быть
 * НАСТРОЕНЫ В ОДНОЙ И ТОЙ ЖЕ СЕТИ NETID — здесь она уже не отвечает за
 * "трансляцию" адресов, а просто объединяет узлы одного проекта.
 *
...
// Собственный адрес ЭТОГО приёмника. ВАЖНО: он отличается от адреса
// репитера (00:02), т.к. репитер пересылает пакеты именно сюда, на 00:03.
#define MY_ADDH      0x00
#define MY_ADDL      0x03
#define LORA_CHANNEL 19    // 850.125 + 19*1 = 869.125 МГц

// Единая сеть NETID для передатчика, репитера и этого приёмника
// (было 0x10 — под аппаратный репитер; теперь у всех узлов одинаковый NETID)
